...
def list_users(
    ...
    """
    Returns all users with the given role, along with computed suspension eligibility stats.

    For students: attendance rate and reschedule count.
    For teachers: number of logged no-shows.
    """
    users = db.query(User).filter(User.role == role).offset(offset).limit(limit).all()
    result = []
    ...
    for user in users:
        ...
        if role == "student":
            ...
            if student:
                ...
                eligible = reschedules >= 3 or attendance < 80
            else:
                attendance = 100
                reschedules = 0
                eligible = False
