...
def get_daily_token(
    ...
    #   nbf  = session start - 10 min  (token isn't valid before the session opens)
    ...
    nbf_delta     = int((sch_at - timedelta(minutes=10) - now).total_seconds())
    ...
    token  = daily.create_meeting_token(
        ...
        nbf_seconds = max(nbf_delta, -600),   # clamp: allow up to 10 min early
    )
